...
- 🔜 Phase 3.4 — iOS: Core/Infrastructure auth foundation (note: after Phase 3.3, the iOS app's existing `NetworkChatService` calls will 401 against chat endpoints until this phase and 3.5 wire up client-side auth — expected, not a regression, per "implement one phase at a time")
